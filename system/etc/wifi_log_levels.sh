...
RESTART_PERIOD=24
RESTART_THRESHOLD=$(($LOOPSTILMETRICS*$RESTART_PERIOD+$RESTART_PERIOD-1))
restartCounter=0
...
function wl_wlan_info ()
{
	...
	CHANNEL=${CHANNEL% *}
	CHANNEL=${CHANNEL%/*}	#in case of channel # followed by a /
	CHANNEL=${CHANNEL%l*}	#in case of channel # followed by a l for lower channel
	CHANNEL=${CHANNEL%u*}	#in case of channel # followed by a u for upper channel
	if [ "$RSSI" -eq 0 ]; then
...
function log_logcat
{
	...
	# Add wl chanim stats output
	IMCOUNTERS=($($WL -i $WLAN_IFACE chanim_stats))
	logStrMetrics="$LOGNAME:"
	logStrMetrics=$logStrMetrics"chan=$CHANNEL;"
	j=3 # skip version, chanspec
	while [[ $j -lt $((${#IMCOUNTERS[*]} - 15)) ]] ; do
		case ${IMCOUNTERS[$j]} in tx|inbss|obss|nocat|nopkt|doze|goodtx|badtx|glitch|badplcp|knoise|idle|timestamp)
			logStrMetrics=$logStrMetrics"${IMCOUNTERS[$j]}=${IMCOUNTERS[$j+15]};"
			;;
		esac
		case ${IMCOUNTERS[$j]} in txop)
			# specify txop in 2g or 5g
			if [[ $CHANNEL -lt 36 ]] ; then
				logStrMetrics=$logStrMetrics"txop_2g=${IMCOUNTERS[$j+15]};"
			else
				logStrMetrics=$logStrMetrics"txop_5g=${IMCOUNTERS[$j+15]};"
			fi
			;;
		esac
		((j++))
...
# Add wl chanmin_stats output into metrics
function log_metrics_wifi_chanim_stats
{
	if [ $CONN_STATUS != "ConnStatusConnected" ] ; then
		return
	fi
	# add chan #
	logStrMetricsLog="$LOGSRC:$LOGNAME:chan$CHANNEL=1;CT;1:NR"
	log -t $METRICSTAG $logStrMetricsLog
	# add other stats
	# CHANCOUNTERS=($($WL -i $WLAN_IFACE chanim_stats))
	k=3 # skip version # and chanspec from "wl chanim_stats" output
	while [[ $k -lt $((${#IMCOUNTERS[*]} - 15)) ]] ; do
		case ${IMCOUNTERS[$k]} in tx|inbss|obss|nocat|nopkt|doze|goodtx|badtx|glitch|badplcp|knoise|idle|timestamp)
			logStrMetricsLog="$LOGSRC:$LOGNAME:${IMCOUNTERS[$k]}${IMCOUNTERS[$k+15]}=1;CT;1:NR"
			log -t $METRICSTAG $logStrMetricsLog
			;;
		esac
		case ${IMCOUNTERS[$k]} in txop)
			# specify txop in 2g or 5g
			if [[ $CHANNEL -lt 36 ]] ; then
				logStrMetricsLog="$LOGSRC:$LOGNAME:txop_2g${IMCOUNTERS[$k+15]}=1;CT;1:NR"
				log -t $METRICSTAG $logStrMetricsLog
			else
				logStrMetricsLog="$LOGSRC:$LOGNAME:txop_5g${IMCOUNTERS[$k+15]}=1;CT;1:NR"
				log -t $METRICSTAG $logStrMetricsLog
			fi
			;;
		esac
...
function run ()
{
	if [[ $restartCounter -ge $RESTART_THRESHOLD ]]; then
		if [[ $currentLoop -eq 0 ]]; then
	...
	BTC27=`$WL btc_params 27`	# check value
	if [ "$WLAN_IFACE" ] ; then
		wl_wlan_info
	fi
	if [ "$P2P_IFACE" ] ; then	# check MCC se
		wl_p2p_info
	fi
